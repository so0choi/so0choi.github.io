...
- `type` : 리스트 순서의 type을 지정할 수 있다. `a`는 영어 소문자, `A`는 영어 대문자 `i`는 로마자 소문자 등 여러가지가 있다. ![](./2020-08-28-15-24-21.png)
